...
class Pix2PixHDModel(torch.nn.Module):
    ...
    def forward(self, src_img, trg_imgs, txt, infer=False):
        """
        :param src_img: (bs, 3, h, w)
        :param trg_imgs: (bs, len, 3, h, w)
        :param txt: (bs, txt_len)
        :return:
        """
        ...
        pseudo_trg_img = []
        valid_len = (trg_imgs[:, :-1].mean((2, 3, 4)) > 0).sum(1)
        for bs_i, valid_len_i in enumerate(valid_len):
            pseudo_trg_img.append(trg_imgs[bs_i, valid_len_i - 1])
        pseudo_trg_img = torch.stack(pseudo_trg_img)
        ...
        # Real Detection and Loss, average over real and pseudo_real
        pred_real = self.discriminate(src_img, trg_img, txt_feat)
        pred_pseudo_real = self.discriminate(src_img, pseudo_trg_img, txt_feat)
        loss_D_real = (self.criterionGAN(pred_real, True) + self.criterionGAN(pred_pseudo_real, True)) / 2
