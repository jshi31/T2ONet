...
def test(model, loader, opt, is_test=False):
    ...
    for i, data in enumerate(loader):
        ...
        with torch.no_grad():
                state, pred_imgs, pred_ops, pred_params = single_model.actor.episode_forward(x, img_x, mask_dict=None,
...
def test_variance(model, loader, opt):
    ...
    for i, data in enumerate(loader):
        ...
        for txt in test_txts:
            ...
            with torch.no_grad():
                state, pred_imgs, pred_ops, pred_params = model.actor.episode_forward(x, img_x, mask_dict=None,
                                                                                reinforce_sample=False)
            # for loop to get column index with end token
            end_fake_imgs = []
            bs, max_len, cn, h, w = pred_imgs.shape
        ...
        if itr % opt.print_every == 0:
            print('iter {:6d} / {}, var {:.4f}, time {:.2f}'.format(itr, len(loader), avg_var, avg_time))
