...
def beam_search(I_0, I_gt, txt, executor, discriminator, beam_size, operations, operation_names, max_step, err, dist_type, optimizer, eps=0.05, replace=False):
    """
    ...
    for i in range(max_step):
        ...
        if random.random() < eps:
            sequences = random.choices(all_candidates, k=beam_size)
        else:
